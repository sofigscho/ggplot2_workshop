...
ggplot(faithful) + 
  geom_point(aes(x = eruptions, y = waiting),
             alpha = 0.2, # 1 completely opaque, 0 completely transparent
             shape = "square",
             size = 5)
```
...
ggplot(mpg) + 
  geom_jitter(aes(x = class, y = hwy), width = 0.2) +
  stat_summary(aes(x = class, y = hwy), geom = "point", fun = mean, colour = "red", size = 3)
```
...
ggplot(mpg) + 
  ...
  scale_colour_brewer(type = 'qual') + 
  scale_size_area(breaks = c(4, 5, 6, 8), max_size = 10) # will use area to scale but ensure that 0 plots to 0 size much more natural progression of sizes, max_size will give more spread in sizes
```
...
#scale_radius you can map it but it cheats the eye
```


`scale_colour_viridis_d()` other type of scale color
* * *
...
```{r}
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy, colour = cty, size = cty)) # continous color mapping
```

```{r}
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy, colour = cty, size = cty))+
  guides(colour = 'legend') #ggplot compresses legend to one, it only merges guides if they are exactly the same
```
...
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy)) + 
  facet_wrap(~ drv, scales = "free") #each plot have their own axis
```
...
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy)) + 
  facet_wrap(~ drv, scales = "free_x") #each plot have their own axis
```
...
ggplot(mpg) + 
  ...
  facet_grid(class ~ .) #space wasting for no data
```
...
ggplot(mpg) + 
  geom_bar(aes(y = manufacturer)) + 
  facet_grid(class ~ ., scales = "free_y") # each plot occupies the same space
```
...
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy)) + 
  facet_wrap( ~ drv + year) #less efective way to visualize it
```
...
ggplot(mpg) + 
  geom_point(aes(x = hwy, y = displ)) +
  scale_y_continuous(trans = "log") # looks bad because breask are calculated before
```

```{r}
ggplot(mpg) + 
  geom_point(aes(x = hwy, y = displ)) +
  coord_trans(y = "log") #look better
```
